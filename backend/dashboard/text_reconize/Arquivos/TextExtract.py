    ...
    def resolve_digits(self):
        ...
        for idx, (bbox, text, prob) in enumerate(resultados):
            ...
            '''if text == '0' or text == '8':
                ...
                if middle_white_ratio > 0.31:  # Reduzir de 0.2 para 0.15 como teste
                    print(f"RATIO  - IF {middle_white_ratio}")
                    print(f"Corrigindo na posição {position}: {text} para 0")
                    text = '0'
                else:
                    print(f"RATIO _ESLE{middle_white_ratio}")
...
class ImageEnhancer:
    ...
    def enhance_image(self, image_path,point):
        ...
        dilated = cv2.erode(dilated, self.kernel_dilate, iterations=9)
        


        novo_tamanho = (dilated.shape[1] * 50, dilated.shape[0] * 130) 
